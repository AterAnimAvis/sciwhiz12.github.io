...
function forEachElementByClassName(name: string, consumer: (HTMLElement) => any) {
    const elements = document.getElementsByClassName(name)
    for (let i = 0; i < elements.length; i++) consumer(elements[i])
}

function createNoQuote(number: number) : HTMLElement {
...
function createQuote(data: Quotes, roles: {[key: string] : Role}, number: number, quote: Quote) {
    ...
    const avatar = createImg(userdata ? "collapse-avatar avatar" : "avatar", quoteDiv, userdata?.avatar)
    avatar.tabIndex = 1
    if (userdata) avatar.alt = "avatar for " + user
    ...
    if (quote?.side_text) {
        ...
        createSpan("side-text", content, quote?.side_text)
    }
